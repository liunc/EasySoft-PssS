...
namespace EasySoft.PssS.DbRepository
{
    ...
    /// 采购配置仓储实现类
    /// </summary>
    public class ExpressCompanyRepository : BaseRepository<ExpressCompany, string>, IExpressCompanyRepository
    {
        ...
        public List<ExpressCompany> Search(string isValid)
        {
            ...
            List<ExpressCompany> entities = new List<ExpressCompany>();
            while (reader.Read())
            {
        ...
        public List<ExpressCompany> Search(string name, int pageIndex, int pageSize, ref int totalCount)
        {
            ...
            if (!string.IsNullOrEmpty(name))
            {
                ...
                para = DbHelper.CreateParameter("Name", DbType.String, Constant.STRING_LENGTH_1);
                para.Value = name;
                paras.Add(para);
            }
            string whereCmdText = string.Empty;
        ...
        protected override ExpressCompany SetEntity(DbDataReader reader)
        {
